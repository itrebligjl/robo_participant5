...
class ExecutionBot(Management):
    ...
    def vwap_orders(self, qty, action, n_slices, exec_t=3.0):
        ...
        while i < n_slices:
            ...
            if i == n_slices - 1:
                ...
                q = min(int(qty_target * volume_profile[i]) + qty_slice,
                        qty)  # qty_slice = possible unfilled size in the previous slice
    ...
    def customized_bots(self, qty, action, n_slices, exec_t=3.0):
        """
        send orders based on volume distribution during a time period, with passive orders followed by aggressive orders
        :param qty: total target qty
        :param action: 'buy' or 'sell'
...
if __name__ == "__main__":
    ...
    #    num_slices, pv, qty = user_bot.ptb_orders(args.size, args.action, args.maxtime, num_slices, beta)
        ...
        pv, qty = exec_bot.customized_bots(args.size, args.action, num_slices, float(args.maxtime / num_slices))
